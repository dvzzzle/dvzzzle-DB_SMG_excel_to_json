        ...
        if sheet_name in xls.sheet_names:
            ...
            if sheet_name == "4 - ОИВ план":
                ...
                if all(column in df_fact.columns for column in copy_columns):
                    ...
                    # Перемещение колонок после "Протяженность"
                    cols = list(df.columns)
                    idx_etazhnost = cols.index("Протяженность")
                    for column in copy_columns:
            ...
            if sheet_name == "5 - ОИВ факт" or sheet_name == '1 - СМГ ежедневный':
                if "ИНН Генподрядчика" in df.columns:
                    df["ИНН Генподрядчика"] = df["ИНН Генподрядчика"].astype(str).str.replace(r'[\s\n\r]+', '', regex=True)
            ...
            # Преобразование значений в колонке "АИП (да/нет)" (если она существует)
            if 'Признак реновации (да/нет)' in df.columns:
                df['Признак реновации (да/нет)'] = df['Признак реновации (да/нет)'].astype(str).str.lower().replace({
                    'true': 'да',
                    'false': 'нет',
                    'да': 'да',
                    'нет': 'нет',
            ...
            for record in data_dict:
                ...
                for key, value in record.items():
                    if pd.notna(value) and isinstance(value, (datetime, pd.Timestamp)):
                        ...
                        value = pd.to_numeric(value, errors='coerce').astype('Int64') if pd.notna(value) else None
                    elif key in ["Общая площадь, м2", "протяженность"]:
...
convert_excel_to_json('E://Загрузки//Telegram Desktop//Текущая обработка//ДСТИИ_для_ДБ_2024_2027_год_31.03.2025_в4.xlsx')
